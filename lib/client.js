...
tytanic.ops.<objectId>.<id (in hex, padded to 64-bits a.k.a. 16 chars)> (the ops we've seen from the server, which are newer than our latest snap.)
...
TODO: support multiple tabs/windows in the same browser by having them listen on localStorage events or something. it might be possible to have only one tab/window use a SockJS connection at a time if a leader election scheme is used (if someone gets elected, they call init(), everyone else at least doesn't do the network portion of init() until they get elected)

TODO: have some convenient way for the user of this library to see if we have localOps pending, so we can display a "saving changes" notification or something if we want.
...
tytanic.client.Client.prototype.requestSnapshot = function(objectId) {
  this.send({
    messageType: 'takeSnapshot',
    objectId: objectId,
  });
};
